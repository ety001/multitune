...
GET    /api/device-info        # 接收并记录车机设备信息（参考 lzc-story）
```
...
后端启动时扫描 `/app/media/` 下的子目录作为可用存储源。用户添加歌曲时，歌单只保存相对/绝对路径索引，不复制文件。
...
### 7.4 权限
- `net.internet`：若需要在线歌词/封面（可选）。
- `user.notify`：播放状态通知（可选）。
- 文件读取权限通过 `binds` 挂载实现。
...
| 数据安全 | 歌单元数据定期备份到 `/lzcapp/var/data/backup/`。 |
